...
class RuntimePolicyResolverTest(unittest.TestCase):
    def test_resolver_prefers_specific_active_binding_and_masks_env_secret(self) -> None:
        with mysql8_session("runtime_policy_resolver", register=register_runtime_policy_tables) as session:
            ...
            for profile in repository.list_active_profiles():
                repository.delete_profile(int(profile["id"]))
