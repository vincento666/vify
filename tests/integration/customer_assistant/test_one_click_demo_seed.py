    ...
    def test_one_click_seed_is_idempotent_and_writes_report_manifest(self) -> None:
        ...
        with _temp_database():
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(manifest["persistence"]["dialect"], "sqlite")
                self.assertEqual(manifest["persistence"]["databaseUrlKind"], "sqlite")
                self.assertFalse(manifest["persistence"]["mysql8LiveRoundTrip"])
