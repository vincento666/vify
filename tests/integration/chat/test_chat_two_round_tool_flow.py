...
class ChatTwoRoundToolFlowTest(unittest.TestCase):
    def test_tool_bound_agent_uses_two_round_tool_flow(self) -> None:
        ...
        self.assertIn("Tool answer:", turn["assistantMessage"]["content"])
        self.assertIn("Order A-100 status: SHIPPED", turn["assistantMessage"]["content"])
    ...
    with get_session_factory()() as session:
        provider_id = session.execute(
            provider.insert().values(
                name=f"Two Round Provider {time.time_ns()}",
                type="OPENAI",
        ...
        model_id = session.execute(
            model_config.insert().values(
                ...
                name="Two Round Model",
                model_id="two-round-model",
                context_size=4096,
                extra_params={},
                enabled=True,
                deleted=False,
        ...
        agent_id = session.execute(
            agent.insert().values(
                name=f"Two Round Agent {time.time_ns()}",
                description="",
                system_prompt="",
        ...
        server_id = session.execute(
            mcp_server.insert().values(
                name=f"Two Round MCP {time.time_ns()}",
                endpoint="mock://tools",
                description="",
                enabled=True,
                deleted=False,
