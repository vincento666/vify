    ...
    with get_session_factory()() as session:
        provider_id = session.execute(
            provider.insert().values(
                name=f"Tool Chat Provider {time.time_ns()}",
                type="OPENAI",
        ...
        agent_id = session.execute(
            agent.insert().values(
                name=f"Tool Chat Agent {time.time_ns()}",
                description="",
                system_prompt="",
        ...
        server_id = session.execute(
            mcp_server.insert().values(
                name=f"Tool Chat MCP {time.time_ns()}",
                endpoint="mock://tool-chat",
                description="",
                enabled=True,
                deleted=False,
                created_at=now,
