...
本项目支持 Claude Code 的 `/loop` 命令：由 `builder` 写代码、`checker` 只检查，编排器循环调度直到全绿或触发刹车。循环必须服从 Hify 的 Spec Kit、TDD、slice 门禁和 `rtk` 命令规范。

### 停止条件

循环在以下任一条件成立时停止：

1. `ALL GREEN`：所有相关检查通过。停止，附上每项检查的通过证明。
2. 轮次用尽：达到 5 轮上限。停止，报告仍失败的项、每轮尝试了什么、为什么没成功。
3. 同一失败连续两轮：builder 可能在猜测而非定位。停止并升级给用户。
4. 回归：修复导致之前通过的检查失败。停止，说明改了什么导致回归。
5. 无实质进展：连续 2 轮失败项数量没有减少。停止，建议拆成更小 slice。
6. 缺少前置门禁：功能边界变化但没有对应 `spec.md`、`plan.md`、`tasks.md`，或缺少 RED 证据。停止，先补 Spec/TDD 记录。
7. 环境或能力边界阻塞：失败原因涉及无法访问的外部依赖、密钥、服务、浏览器 UAT 条件或数据库环境。停止，报告阻塞点。
...
- 永远不覆盖用户已有的未相关改动。
- 永远不绕过前端 rem 门禁、浏览器 UAT 或 Docs 证据要求。
