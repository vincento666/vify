...
try {
  ...
  assert(panelText.includes('变量名称'), 'Assignment editor must show a target variable column')
  assert(panelText.includes('类型'), 'Assignment editor must show source type column')
  assert(panelText.includes('值'), 'Assignment editor must show assignment value column')
  ...
  assert(await panel.getByLabel('赋值来源类型', { exact: true }).count() === 1, 'Expected assignment row to expose source type selector')
  assert(await panel.getByLabel('赋值来源类型', { exact: true }).inputValue() === 'reference', 'Expected referenced assignment to select 引用 source type')
