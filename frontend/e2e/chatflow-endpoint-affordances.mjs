...
try {
  ...
  assert(closeToRatio(nodeHoverSource.dotScale, 1), `Expected node hover port dot scale 1x, got ${JSON.stringify(nodeHoverSource)}`)
  ...
  assert(closeToRatio(endpointHoverSource.dotScale, 1.2), `Expected endpoint hover dot scale 1.2x, got ${JSON.stringify(endpointHoverSource)}`)
  ...
  assert(
    closeToRatio(radiusHoverSource.dotScale, 1.2),
    `Expected endpoint hover radius to trigger 1.2x scale inside the 3x original diameter, got ${JSON.stringify(radiusHoverSource)}`,
  )
  ...
  assert(
    closeToRatio(outsideRadiusSource.dotScale, 1),
    `Expected endpoint hover to fall back to node-hover 1x outside the 3x original diameter, got ${JSON.stringify(outsideRadiusSource)}`,
  )
  ...
  assert(closeToRatio(selectedSource.dotScale, 1), `Expected selected node port dot scale 1x, got ${JSON.stringify(selectedSource)}`)
