...
| 213.2.1 | **Migrate chatflow-run-optimistic-loading e2e to async runtime mock surface**（扩大范围）：原计划仅刷新 mock URL `/runs-legacy` → `/runs-v2`；执行中暴露 mock response body 仍是 legacy sync envelope（`status: SUCCEEDED, inline output, streamEvents`），production V2 runner 拿到后会继续 poll `/runtime-runs/{id}` 状态/事件/结果导致 `Runtime v2 run not found`。完整迁移：mock URL + mock fulfill body 改 six-ref envelope + 加 `/runtime-runs/{id}` (status) / `/runtime-runs/{id}/events` / `/runtime-runs/{id}/result` (+ 可选 events/stream SSE) mock stub | RED / E2E / UAT / Docs |
...
| 213.3 | SOP Router 调用 Chatflow 默认返回 runtime refs；SOP Router state 仅保存 conversation/active-child/suspended/route history/resume offer/intent summary（**拆分 6 个 sub-slice**：213.3.1 加 ref columns + dual-write / 213.3.2 暴露 runtime refs on /messages envelope / 213.3.3 替换 _session_business_context 为 child-Chatflow aggregator / 213.3.4 停止写禁字段 / 213.3.5 drop checkpoint table 和 banned columns / 213.3.6 deprecate FakeSopRuntimeAdapter；详见 `artifacts/.../213.3/AUDIT.md`） | RED / Unit / Integration / E2E / UAT |
