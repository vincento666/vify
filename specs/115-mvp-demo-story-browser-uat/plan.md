...
4. Run the one-click seed against a disposable MySQL8 UAT schema, then run
   `scripts/dev.sh` with the same MySQL8 database.
...
- Seed: `env PYTHONPATH=. HIFY_DATABASE_URL=mysql+pymysql://hify:hify@127.0.0.1:3306/hify?charset=utf8mb4 uv run python scripts/seed_one_click_mvp_demo.py`.
...
- Shared default dev database state can make confirm paths non-repeatable, so
  115 uses a reset disposable MySQL8 schema for seed and dev server.
