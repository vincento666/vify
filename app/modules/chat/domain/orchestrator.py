...
class ChatOrchestrator:
    ...
    def run(
        ...
        first_payload = self._request_builder.build(model=model, messages=messages, tools=tools)
        first = self._parser.parse_chat_response(self._llm_client.complete(first_payload))
        ...
        second_messages = [
            *messages,
            ChatRequestMessage(role="assistant", content=first.content),
            *[
                ChatRequestMessage(
                    role="tool",
        ...
        second_payload = self._request_builder.build(model=model, messages=second_messages)
        second = self._parser.parse_chat_response(self._llm_client.complete(second_payload))
