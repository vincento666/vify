...
class ToolRunner:
    ...
    def run(self, tool_name: str, payload: dict[str, Any], *, idempotency_key: str = "") -> ToolRunResult:
        ...
        for attempt in range(1, max_attempts + 1):
            attempts = attempt
            dispatch_payload = _payload_with_runtime(payload, resolved_idempotency_key, attempt)
            try:
...
def _payload_with_runtime(payload: dict[str, Any], idempotency_key: str, attempt: int) -> dict[str, Any]:
